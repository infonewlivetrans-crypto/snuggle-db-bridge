            ...
            (c) => (
              <li key={c.id} className="rounded border border-border bg-background p-2 text-sm">
                <div className="mb-0.5 text-xs text-muted-foreground">
                  <span suppressHydrationWarning>{new Date(c.created_at).toLocaleString("ru-RU")}</span> · {c.author_name ?? "—"}
                </div>
                <div>{c.body}</div>
              </li>
            ),
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
              <span><span suppressHydrationWarning>{new Date(task.created_at).toLocaleString("ru-RU")}</span></span>
