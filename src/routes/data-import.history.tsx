                  ...
                  {logs.map((l) => (
                    <TableRow key={l.id} className="cursor-pointer" onClick={() => setOpenLog(l)}>
                      <TableCell className="whitespace-nowrap text-sm">
                        <span suppressHydrationWarning>{new Date(l.created_at).toLocaleString("ru-RU")}</span>
                      </TableCell>
      ...
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-auto">
        {log && (
          <>
            <DialogHeader>
              <DialogTitle>
                Импорт {entityLabel(log.entity)} от <span suppressHydrationWarning>{new Date(log.created_at).toLocaleString("ru-RU")}</span>
              </DialogTitle>
              <DialogDescription>
