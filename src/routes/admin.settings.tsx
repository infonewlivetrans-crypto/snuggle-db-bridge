...
function ModuleTogglesPanel({
  ...
  let value: Partial<EnabledModules> = {};
  ...
  if (typeof rawValue === "string") {
    try {
      ...
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
        value = parsed as Partial<EnabledModules>;
      }
    } catch {
      value = {};
    }
  } else if (rawValue && typeof rawValue === "object" && !Array.isArray(rawValue)) {
    value = rawValue as Partial<EnabledModules>;
  }
...
function SettingsList({ items, onChanged }: { items: SystemSetting[]; onChanged: () => void }) {
  const grouped = useMemo(() => {
    ...
    return Array.from(map.entries()).sort((a, b) => String(a[0] ?? "").localeCompare(String(b[0] ?? ""), "ru"));
  }, [items]);
      ...
      <CardContent className="space-y-2">
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs text-muted-foreground">
            Обновлено: <span suppressHydrationWarning>{new Date(setting.updated_at).toLocaleString("ru-RU")}</span>
          </span>
