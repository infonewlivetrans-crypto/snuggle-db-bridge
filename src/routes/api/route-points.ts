    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          if (idsList.length === 0 && orderIds.length === 0 && !createdToday)
            return jsonResponse(
              { error: "route_id or order_id_in or created_today required" },
              { status: 400 },
            );
          ...
          if (error) {
            console.error("/api/route-points GET supabase error:", {
          ...
          console.error("/api/route-points GET unhandled:", { msg, stack });
          return jsonResponse({ error: `unhandled: ${msg}` }, { status: 500 });
        }
      },
      ...
      POST: async ({ request }) => {
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          console.error("/api/route-points POST unhandled:", e);
          return jsonResponse({ error: `unhandled: ${msg}` }, { status: 500 });
        }
      },
      ...
      DELETE: async ({ request }) => {
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          console.error("/api/route-points DELETE unhandled:", e);
          return jsonResponse({ error: `unhandled: ${msg}` }, { status: 500 });
        }
      },
    },
