    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          ...
          const [routesCreatedToday, routesCompletedToday, ordersProcessedToday, errorsToday, pointsClosedToday, reportsToday, driverLocationsToday] = await Promise.all([
          ...
          for (const row of (todayAudit ?? []) as Array<{ user_id: string | null; user_role: string | null }>) {
          ...
          const inactiveToday = ((allProfiles ?? []) as Array<{ user_id: string; full_name: string | null; email: string | null }>)
            .filter((p) => !activeUserIds.has(p.user_id))
            .map((p) => ({ userId: p.user_id, name: p.full_name, email: p.email, roles: rolesByUser.get(p.user_id) ?? [] }))
            .slice(0, 100);
          ...
          for (const r of (weekAudit ?? []) as Array<{ created_at: string; user_id: string | null }>) {
            ...
            if (b) { b.actions += 1; if (r.user_id) b.users.add(r.user_id); }
          }
          const weekChart = Array.from(dayBuckets.entries()).map(([date, v]) => ({ date, actions: v.actions, users: v.users.size }));
          ...
          return jsonResponse({
            today,
            kpi: { usersToday: activeUserIds.size, routesCreatedToday, routesCompletedToday, ordersProcessedToday, errorsToday, pointsClosedToday, reportsToday },
            byRole: { driver: byRole.driver ?? 0, logist: byRole.logist ?? 0, manager: byRole.manager ?? 0, warehouse: byRole.warehouse ?? 0, director: byRole.director ?? 0, admin: byRole.admin ?? 0, supply: byRole.supply ?? 0 },
            inactiveToday,
            inactiveTotal: ((allProfiles ?? []).length) - activeUserIds.size > 0 ? (allProfiles ?? []).length - activeUserIds.size : 0,
            warnings,
            weekChart,
          });
        } catch (e) {
