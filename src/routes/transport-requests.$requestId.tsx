...
function TransportRequestDetailPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<RequestDetail | null> => {
      return await apiGetAuth<RequestDetail | null>(
        `/api/routes/${encodeURIComponent(requestId)}`,
      );
    },
  });

  const { data: totals } = useQuery({
    queryKey: ["transport-request-page-totals", requestId],
    queryFn: async () => {
      const totals = await apiGetAuth<{
        totalWeight: number;
        totalVolume: number;
        ordersCount: number;
          ...
          <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="ml-auto flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  ...
                  <span className="inline-flex items-center gap-1">
                    ...
                    <span suppressHydrationWarning>{new Date(data.created_at).toLocaleDateString("ru-RU")}</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                {data.route_date ? (
                  <span><span suppressHydrationWarning>{new Date(data.route_date).toLocaleDateString("ru-RU")}</span></span>
                ) : (
            ...
            <Section title="Основное">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Field
                  label="Дата"
                  value=<span suppressHydrationWarning>{new Date(data.route_date).toLocaleDateString("ru-RU")}</span>
                />
                <Field
                  label="Вид заявки"
