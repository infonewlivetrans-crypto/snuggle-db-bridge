...
function DeliveryRoutesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Row[]> => {
      return await apiGetAuth<Row[]>("/api/delivery-routes-list");
    },
  });
                ...
                filtered.map((r) => {
                  ...
                  return (
                    <TableRow key={r.id} className="cursor-pointer">
                      ...
                      <TableCell><span suppressHydrationWarning>{new Date(r.route_date).toLocaleDateString("ru-RU")}</span></TableCell>
                      <TableCell>
                        {r.source_request ? (
                          <Link
