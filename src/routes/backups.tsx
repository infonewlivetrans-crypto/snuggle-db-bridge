        ...
        {isStale ? (
          <div className="mb-4 flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 p-4 text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
            ...
            <div className="text-sm">
              ...
              <div>
                {lastSuccess
                  ? `Последняя успешная копия: $<span suppressHydrationWarning>{new Date(lastSuccess.created_at).toLocaleString("ru-RU")}</span>.`
                ...
                (data ?? []).map((b) => (
                  <TableRow key={b.id}>
                    <TableCell className="whitespace-nowrap text-xs">
                      <span suppressHydrationWarning>{new Date(b.created_at).toLocaleString("ru-RU")}</span>
                    </TableCell>
                    <TableCell>
            ...
            <div className="space-y-3 text-sm">
              {restoreTarget ? (
                <div className="rounded-md border border-border bg-secondary/40 p-3 text-xs">
                  Копия от: <span className="font-medium"><span suppressHydrationWarning>{new Date(restoreTarget.created_at).toLocaleString("ru-RU")}</span></span>
                </div>
              ) : null}
