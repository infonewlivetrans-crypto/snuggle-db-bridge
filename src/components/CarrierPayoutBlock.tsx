          ...
          <div className="flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">
            ...
            <div>
              ...
              <div className="text-xs mt-0.5">
                Запланировано на{" "}
                <span suppressHydrationWarning>{new Date(row.carrier_payout_scheduled_date!).toLocaleDateString("ru-RU")}</span>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="text-lg font-semibold">
              ...
              {row.carrier_payout_paid_at && (
                <span className="ml-2 text-xs text-muted-foreground">
                  • <span suppressHydrationWarning>{new Date(row.carrier_payout_paid_at).toLocaleDateString("ru-RU")}</span>
                </span>
              )}
            </div>
          </div>
