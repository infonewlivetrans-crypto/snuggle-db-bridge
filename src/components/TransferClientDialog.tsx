  ...
  const undoMut = useMutation({
    mutationFn: async () => {
      ...
      const entry: TransferHistoryEntry = {
        ...
        transferred_by_user_id: user?.id ?? null,
        transferred_by_name: profile?.full_name ?? user?.email ?? null,
        reason: `Отмена передачи от $<span suppressHydrationWarning>{new Date(last.at).toLocaleString("ru-RU")}</span>`,
        at: new Date().toISOString(),
      };
                  ...
                  <li key={i} className="border-l-2 border-border pl-2">
                    <div>
                      <span className="text-muted-foreground">
                        <span suppressHydrationWarning>{new Date(h.at).toLocaleString("ru-RU")}</span>
                      </span>
                    </div>
                    <div>
                      {h.from_manager_name ?? "—"} →{" "}
