...
const NAV_ITEMS = [
  ...
  { to: "/data-import", label: "Импорт данных", icon: FileSpreadsheet, match: (p: string) => p.startsWith("/data-import") && !p.startsWith("/data-import/history") },
  { to: "/data-import/history", label: "История импорта", icon: FileSpreadsheet, match: (p: string) => p.startsWith("/data-import/history") },
  ...
  { to: "/pilot", label: "Пилотный запуск", icon: PlayCircle, match: (p: string) => p.startsWith("/pilot") },
