              ...
              <SelectContent>
                {(routes ?? []).map((r) => (
                  <SelectItem key={r.id} value={r.id} disabled={usedRouteIds.has(r.id)}>
                    {r.route_number} · <span suppressHydrationWarning>{new Date(r.route_date).toLocaleDateString("ru-RU")}</span>
          ...
          {selected && (
            <div className="rounded-lg border border-border bg-secondary/40 p-3 text-sm">
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                Дата: <span suppressHydrationWarning>{new Date(selected.route_date).toLocaleDateString("ru-RU")}</span>
