...
CREATE POLICY "delivery_routes_carrier_select" ON public."delivery_routes" AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id IS NOT NULL AND p.carrier_id = delivery_routes.carrier_id)));
DROP POLICY IF EXISTS "delivery_routes_carrier_update" ON public."delivery_routes";
CREATE POLICY "delivery_routes_carrier_update" ON public."delivery_routes" AS PERMISSIVE FOR UPDATE TO authenticated USING ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id IS NOT NULL AND p.carrier_id = delivery_routes.carrier_id)));
...
CREATE POLICY "orders_carrier_select" ON public."orders" AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS (SELECT 1 FROM public.route_points rp JOIN public.delivery_routes dr ON dr.source_request_id = rp.route_id JOIN public.profiles p ON p.user_id = auth.uid() WHERE rp.order_id = orders.id AND p.carrier_id IS NOT NULL AND p.carrier_id = dr.carrier_id)));
...
CREATE POLICY "rcd_carrier_delete" ON public."route_carrier_documents" AS PERMISSIVE FOR DELETE TO authenticated USING ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id = route_carrier_documents.carrier_id)));
DROP POLICY IF EXISTS "rcd_carrier_insert" ON public."route_carrier_documents";
CREATE POLICY "rcd_carrier_insert" ON public."route_carrier_documents" AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id = route_carrier_documents.carrier_id)));
DROP POLICY IF EXISTS "rcd_carrier_select" ON public."route_carrier_documents";
CREATE POLICY "rcd_carrier_select" ON public."route_carrier_documents" AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id = route_carrier_documents.carrier_id)));
...
CREATE POLICY "rch_carrier_select" ON public."route_carrier_history" AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id = route_carrier_history.carrier_id)));
...
CREATE POLICY "route_offers_carrier_select" ON public."route_offers" AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id = route_offers.carrier_id)));
DROP POLICY IF EXISTS "route_offers_carrier_update" ON public."route_offers";
CREATE POLICY "route_offers_carrier_update" ON public."route_offers" AS PERMISSIVE FOR UPDATE TO authenticated USING ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id = route_offers.carrier_id)));
...
CREATE POLICY "route_points_carrier_select" ON public."route_points" AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS (SELECT 1 FROM public.delivery_routes dr JOIN public.profiles p ON p.user_id = auth.uid() WHERE dr.source_request_id = route_points.route_id AND p.carrier_id IS NOT NULL AND p.carrier_id = dr.carrier_id)));
DROP POLICY IF EXISTS "route_points_carrier_update" ON public."route_points";
CREATE POLICY "route_points_carrier_update" ON public."route_points" AS PERMISSIVE FOR UPDATE TO authenticated USING ((EXISTS (SELECT 1 FROM public.delivery_routes dr JOIN public.profiles p ON p.user_id = auth.uid() WHERE dr.source_request_id = route_points.route_id AND p.carrier_id IS NOT NULL AND p.carrier_id = dr.carrier_id)));
...
CREATE POLICY "routes_carrier_select_own" ON public."routes" AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS (SELECT 1 FROM public.profiles p WHERE p.user_id = auth.uid() AND p.carrier_id IS NOT NULL AND p.carrier_id = routes.carrier_id)));
