ALTER TABLE public.route_points
  ADD COLUMN IF NOT EXISTS dp_amount_received numeric,
  ADD COLUMN IF NOT EXISTS dp_payment_comment text;
...
BEGIN
  ...
  IF NEW.dp_status = 'delivered' THEN
    ...
    v_body := 'Заказ №' || v_order.order_number || ' доставлен.' || v_qr_line || v_payment_line;
  ELSIF NEW.dp_status = 'not_delivered' THEN
    ...
    v_reason_label := CASE NEW.dp_undelivered_reason::text
      ...
      ELSE 'не указана'
    END;
    v_body := 'Заказ №' || v_order.order_number || ' не доставлен. Причина: ' || v_reason_label || '.' || v_qr_line || v_payment_line;
  ELSE
    ...
    v_body := 'Заказ №' || v_order.order_number ||
              ...
              COALESCE(' «' || v_wh_name || '»','') ||
              '. Ожидаемое время возврата: ' || v_expected || '.' || v_qr_line || v_payment_line;
  END IF;
