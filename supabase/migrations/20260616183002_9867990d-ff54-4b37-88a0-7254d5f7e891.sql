-- Carrier self-register: idempotent (grants role + dcu even if profile already exists)
-- + carrier_self_heal() to backfill role/dcu for users that registered earlier
-- while the RPC was bailing in the "already linked" branch.

CREATE OR REPLACE FUNCTION public.carrier_self_register(payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  v_user_id uuid := auth.uid();
  v_email text := lower(trim(coalesce(payload->>'email', '')));
  v_company text := trim(coalesce(payload->>'company_name', ''));
  v_kind text := coalesce(payload->>'carrier_kind', 'self_employed');
  v_carrier_type public.carrier_type;
  v_inn text := nullif(trim(coalesce(payload->>'inn','')), '');
  v_ogrn text := nullif(trim(coalesce(payload->>'ogrn','')), '');
  v_phone text := nullif(trim(coalesce(payload->>'phone','')), '');
  v_city text := nullif(trim(coalesce(payload->>'city','')), '');
  v_contact text := nullif(trim(coalesce(payload->>'contact_person','')), '');
  v_pay_method text := nullif(trim(coalesce(payload->>'commission_payment_method','')), '');
  v_agreed_by text := nullif(trim(coalesce(payload->>'commission_agreed_by','')), '');
  v_reg_type text := coalesce(payload->>'registration_type', 'carrier_only');
  v_driver_name text := nullif(trim(coalesce(payload->>'driver_full_name','')), '');
  v_driver_phone text := nullif(trim(coalesce(payload->>'driver_phone','')), '');
  v_existing_carrier uuid;
  v_existing_ext uuid;
  v_carrier_id uuid;
  v_ext_id uuid;
  v_driver_id uuid;
BEGIN
  IF v_user_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'unauthorized');
  END IF;
  IF v_email = '' OR v_company = '' OR v_phone IS NULL OR v_agreed_by IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'validation_failed');
  END IF;

  v_carrier_type := CASE lower(v_kind)
    WHEN 'ip' THEN 'ip'::public.carrier_type
    WHEN 'ooo' THEN 'ooo'::public.carrier_type
    WHEN 'self_employed' THEN 'self_employed'::public.carrier_type
    WHEN 'individual' THEN 'self_employed'::public.carrier_type
    ELSE NULL
  END;
  IF v_carrier_type IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'validation_failed', 'field', 'carrier_kind');
  END IF;

  SELECT carrier_id INTO v_existing_carrier
  FROM public.profiles WHERE user_id = v_user_id;

  IF v_existing_carrier IS NOT NULL THEN
    -- Found a profile already linked. Make sure role + dcu link exist anyway.
    SELECT id INTO v_existing_ext
    FROM public.dispatcher_carrier_ext
    WHERE carrier_id = v_existing_carrier
    LIMIT 1;

    INSERT INTO public.user_roles(user_id, role)
    VALUES (v_user_id, 'carrier'::public.app_role)
    ON CONFLICT (user_id, role) DO NOTHING;

    IF v_existing_ext IS NOT NULL THEN
      IF NOT EXISTS (
        SELECT 1 FROM public.dispatcher_carrier_users
        WHERE user_id = v_user_id
          AND dispatcher_carrier_ext_id = v_existing_ext
          AND status = 'active'
      ) THEN
        INSERT INTO public.dispatcher_carrier_users(
          dispatcher_carrier_ext_id, user_id, status, created_by
        ) VALUES (v_existing_ext, v_user_id, 'active', v_user_id)
        ON CONFLICT DO NOTHING;
      END IF;
    END IF;

    RETURN jsonb_build_object('ok', true, 'already_linked', true, 'carrier_id', v_existing_carrier);
  END IF;

  INSERT INTO public.carriers (carrier_type, company_name, inn, ogrn, phone, email, city, contact_person, verification_status, source)
  VALUES (v_carrier_type, v_company, v_inn, v_ogrn, v_phone, v_email, v_city, v_contact, 'new', 'carrier_self_register')
  RETURNING id INTO v_carrier_id;

  INSERT INTO public.dispatcher_carrier_ext (
    carrier_id, name, carrier_kind, inn, ogrn, phone, email, city,
    commission_rate, commission_agreed, commission_agreed_at, commission_agreed_by,
    commission_agreement_text, commission_payment_method, verification_status
  ) VALUES (
    v_carrier_id, v_company, v_kind, v_inn, v_ogrn, v_phone, v_email, v_city,
    0.05, true, now(), v_agreed_by,
    'Я подтверждаю, что за рейсы, найденные диспетчером/сервисом, оплачиваю комиссию 5% после получения оплаты за перевозку.',
    v_pay_method, 'new'
  ) RETURNING id INTO v_ext_id;

  INSERT INTO public.profiles (user_id, full_name, email, phone, carrier_id, is_active)
  VALUES (v_user_id, coalesce(v_contact, v_company), v_email, v_phone, v_carrier_id, true)
  ON CONFLICT (user_id) DO UPDATE
    SET carrier_id = COALESCE(public.profiles.carrier_id, EXCLUDED.carrier_id),
        phone = COALESCE(public.profiles.phone, EXCLUDED.phone),
        email = COALESCE(public.profiles.email, EXCLUDED.email),
        is_active = true;

  INSERT INTO public.user_roles(user_id, role)
  VALUES (v_user_id, 'carrier'::public.app_role)
  ON CONFLICT (user_id, role) DO NOTHING;

  UPDATE public.dispatcher_carrier_users
    SET status = 'blocked'
    WHERE status = 'active'
      AND user_id = v_user_id
      AND dispatcher_carrier_ext_id <> v_ext_id;

  INSERT INTO public.dispatcher_carrier_users(
    dispatcher_carrier_ext_id, user_id, status, created_by
  ) VALUES (v_ext_id, v_user_id, 'active', v_user_id)
  ON CONFLICT DO NOTHING;

  IF v_reg_type = 'carrier_with_driver' AND v_driver_name IS NOT NULL AND v_driver_phone IS NOT NULL THEN
    INSERT INTO public.drivers (full_name, phone, city, carrier_id, is_active)
    VALUES (v_driver_name, v_driver_phone, v_city, v_carrier_id, true)
    RETURNING id INTO v_driver_id;

    INSERT INTO public.dispatcher_driver_ext (
      driver_id, full_name, phone, city, dispatcher_carrier_ext_id, dispatcher_status, docs_status
    ) VALUES (
      v_driver_id, v_driver_name, v_driver_phone, v_city, v_ext_id, 'new', 'not_uploaded'
    );
  END IF;

  INSERT INTO public.dispatcher_tasks (
    task_type, title, description, priority, task_status,
    related_entity_type, related_entity_id, dispatcher_carrier_ext_id
  ) VALUES (
    'check_documents',
    'Проверить нового перевозчика: ' || v_company,
    'Перевозчик зарегистрировался самостоятельно через /carrier/register. Email: ' || v_email || ', Телефон: ' || coalesce(v_phone,''),
    'normal', 'open', 'carrier', v_carrier_id, v_ext_id
  );

  RETURN jsonb_build_object('ok', true, 'carrier_id', v_carrier_id, 'already_linked', false);
END;
$function$;

REVOKE ALL ON FUNCTION public.carrier_self_register(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.carrier_self_register(jsonb) TO authenticated;

-- Self-heal RPC: backfills role='carrier' and active dispatcher_carrier_users row
-- for the current user when profiles.carrier_id is already set.
-- Safe to call on every cabinet load. No-ops if nothing to fix.
CREATE OR REPLACE FUNCTION public.carrier_self_heal()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_user uuid := auth.uid();
  v_carrier uuid;
  v_ext uuid;
  v_changed boolean := false;
BEGIN
  IF v_user IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'unauthorized');
  END IF;

  SELECT carrier_id INTO v_carrier FROM public.profiles WHERE user_id = v_user LIMIT 1;
  IF v_carrier IS NULL THEN
    RETURN jsonb_build_object('ok', true, 'reason', 'no_profile_carrier');
  END IF;

  -- Ensure carrier role
  IF NOT EXISTS (
    SELECT 1 FROM public.user_roles WHERE user_id = v_user AND role = 'carrier'::public.app_role
  ) THEN
    INSERT INTO public.user_roles(user_id, role) VALUES (v_user, 'carrier'::public.app_role)
    ON CONFLICT DO NOTHING;
    v_changed := true;
  END IF;

  -- Ensure dispatcher_carrier_users link
  SELECT id INTO v_ext FROM public.dispatcher_carrier_ext WHERE carrier_id = v_carrier LIMIT 1;
  IF v_ext IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.dispatcher_carrier_users
    WHERE user_id = v_user
      AND dispatcher_carrier_ext_id = v_ext
      AND status = 'active'
  ) THEN
    INSERT INTO public.dispatcher_carrier_users(dispatcher_carrier_ext_id, user_id, status, created_by)
    VALUES (v_ext, v_user, 'active', v_user)
    ON CONFLICT DO NOTHING;
    v_changed := true;
  END IF;

  RETURN jsonb_build_object('ok', true, 'changed', v_changed, 'carrier_id', v_carrier, 'ext_id', v_ext);
END;
$$;

REVOKE ALL ON FUNCTION public.carrier_self_heal() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.carrier_self_heal() TO authenticated;

-- Backfill historical users: those who have profiles.carrier_id but no role / no dcu link.
INSERT INTO public.user_roles(user_id, role)
SELECT p.user_id, 'carrier'::public.app_role
FROM public.profiles p
WHERE p.carrier_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.user_roles ur
    WHERE ur.user_id = p.user_id AND ur.role = 'carrier'::public.app_role
  )
ON CONFLICT DO NOTHING;

INSERT INTO public.dispatcher_carrier_users(dispatcher_carrier_ext_id, user_id, status, created_by)
SELECT e.id, p.user_id, 'active', p.user_id
FROM public.profiles p
JOIN public.dispatcher_carrier_ext e ON e.carrier_id = p.carrier_id
WHERE p.carrier_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.dispatcher_carrier_users dcu
    WHERE dcu.user_id = p.user_id
      AND dcu.dispatcher_carrier_ext_id = e.id
      AND dcu.status = 'active'
  )
ON CONFLICT DO NOTHING;