...
BEGIN
  ...
  SELECT id, order_number, contact_name, delivery_address, amount_due, requires_qr, qr_received, comment
    INTO v_order
  ...
  IF NEW.dp_unload_started_at IS NOT NULL AND NEW.dp_unload_finished_at IS NOT NULL THEN
    v_unload_minutes := GREATEST(0, EXTRACT(EPOCH FROM (NEW.dp_unload_finished_at - NEW.dp_unload_started_at))::INT / 60);
  END IF;

  IF NEW.dp_status = 'delivered' THEN
    v_kind := 'order_delivered';
    v_title := 'Заказ доставлен';
    v_status_label := 'Доставлено';
  ...
  ELSE
    ...
    v_status_label := 'Возврат на склад';
  ...
  -- Дополнительное уведомление: «Отчёт по доставке»
  v_report_payload := jsonb_build_object(
    ...
    'manager_name', v_manager,
    'final_status', NEW.dp_status,
    'final_status_label', v_status_label,
    'reason', NEW.dp_undelivered_reason,
    'reason_label', v_reason_label,
    'return_warehouse_name', v_wh_name,
    'return_comment', NEW.dp_return_comment,
    'expected_return_at', NEW.dp_expected_return_at,
    'arrived_at', NEW.dp_actual_arrival_at,
    'unload_started_at', NEW.dp_unload_started_at,
    'unload_finished_at', NEW.dp_unload_finished_at,
    'finished_at', NEW.dp_finished_at,
    'unload_minutes', v_unload_minutes,
    'idle_duration_minutes', NEW.dp_idle_duration_minutes,
    'idle_reason', NEW.dp_idle_reason,
    'idle_reason_label', v_idle_reason_label,
    'idle_comment', NEW.dp_idle_comment,
  ...
  VALUES (
    'delivery_report',
    'Отчёт по доставке',
    'Отчёт по заказу №' || v_order.order_number || ' — ' || v_status_label || '.',
    NEW.order_id,
    v_report_payload
  );
