DROP FUNCTION IF EXISTS public.driver_update_order_payment(uuid, boolean, boolean, text);

CREATE FUNCTION public.driver_update_order_payment(
  p_order_id uuid,
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    JOIN public.delivery_routes dr
      ON dr.source_request_id = rp.route_id
  ...
  UPDATE public.orders o
     SET cash_received = COALESCE(p_cash_received, o.cash_received),
         qr_received = COALESCE(p_qr_received, o.qr_received),
         payment_status = COALESCE(p_payment_status, o.payment_status)
   WHERE o.id = p_order_id
   RETURNING o.id, o.cash_received, o.qr_received, o.payment_status
